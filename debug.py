...
def debug_cuboid_points(path_img, path_json, path_output, img_name):
...
if __name__ == "__main__":
    ...
    for i, (img_path, img_name, json_path) in enumerate(imgs):
        print(f"Debugging image {i + 1} of {len(imgs)}")
        debug_cuboid_points(img_path, json_path, opt.outf, img_name)
